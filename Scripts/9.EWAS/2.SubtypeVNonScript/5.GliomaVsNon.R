...
# Load covariate data
targets <- read.csv("./Files/Metadata3.csv", stringsAsFactors = FALSE, row.names = 1)
...
Glioma <- targets %>% filter(Tumor_Type == "Glioma")
dim(Glioma)
NonTumor <-targets %>% filter(Tumor_Type == "Non-Tumor")
dim(NonTumor)
GliomavNonT <- rbind(Glioma, NonTumor)


#add ID to rownames of targets
row.names(GliomavNonT) <- GliomavNonT$Label.ID.


Betas <- UniqueSamplesTop5CpGs[ , colnames(UniqueSamplesTop5CpGs) %in% rownames(GliomavNonT)];
...
if(identical(row.names(GliomavNonT),colnames(Betas))) {
  ...
  GliomavNonT <- GliomavNonT[match(colnames(Betas), row.names(GliomavNonT)), ];
}
...
stopifnot( identical(row.names(GliomavNonT), colnames(Betas)) );


## Preview your covariates before building the linear model
table(GliomavNonT$isMale);
table(GliomavNonT$isTumor);

hist(GliomavNonT$DX.AGE);
qplot(as.numeric(Betas), geom="histogram", xlab="5hmC Beta values", ylab="Count")


##Create Design Matrix:
design_istumor <- model.matrix( ~ isTumor + isMale + DX.AGE, data=GliomavNonT);
...
write.csv(DMP_istumor, file="./Script/15.EWAS/SubtypeToNonDMR/BetasGliomaVSNon_results030120.csv", quote=FALSE);
