...
#Subset to Embryonal tumors and Non-Tumors
Emb <- targets %>% filter(Tumor_Type == "Embryonal")
dim(Emb)
NonTumor <-targets %>% filter(Tumor_Type == "Non-Tumor")
dim(NonTumor)
EmbvNonT <- rbind(Emb, NonTumor)


#add ID to rownames of targets
row.names(EmbvNonT) <- EmbvNonT$Label.ID.


Betas <- UniqueSamplesTop5CpGs[ , colnames(UniqueSamplesTop5CpGs) %in% rownames(EmbvNonT)];
...
if(identical(row.names(EmbvNonT),colnames(Betas))) {
  ...
  EmbvNonT <- EmbvNonT[match(colnames(Betas), row.names(EmbvNonT)), ];
}
...
stopifnot( identical(row.names(EmbvNonT), colnames(Betas)) );


## Preview your covariates before building the linear model
table(EmbvNonT$isMale);
table(EmbvNonT$isTumor);

hist(EmbvNonT$DX.AGE);
qplot(as.numeric(Betas), geom="histogram", xlab="5hmC Beta values", ylab="Count")


##Create Design Matrix:
design_istumor <- model.matrix( ~ isTumor + isMale + DX.AGE, data=EmbvNonT);
...
write.csv(DMP_istumor, file="./Plots/0.052020_wPCNS0601X/SubtypeEWAS/BetasEmbVSNon_results0520.csv", quote=FALSE);
