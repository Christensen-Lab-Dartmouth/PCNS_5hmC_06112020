...
EpendVNon <- read.csv("./Plots/0.052020_wPCNS0601X/SubtypeEWAS/BetasEpendVSNon_results0520.csv", stringsAsFactors = F, row.names =1)
qsum <- EpendVNon %>% filter(adj.P.Val < 0.1)
sum(qsum$logFC < 0)
...
#  scale_x_continuous(limits=c(-0.5,0.4), breaks=seq(-0.4,0.4,by=.1)) +
 # scale_y_continuous(limits=c(0,2)) +
  ggtitle("Embryonal Tumor vs. Nontumor 5hmC ");


# -------------------------- Step 4. Data Visualization of unadjusted P. Values --------------------------
plt_istumor2 <- EpendVNon
...
plt_istumor2$dir[plt_istumor2$adj.P.Val > PVAL_THRESH] <- "(Not Significant)";
...
plt_istumor$dir <- ifelse(plt_istumor$logFC > 0, "Hyper-hydroxymethylated", "Hypo-hydroxymethylated");
plt_istumor$dir[plt_istumor$adj.P.Val > PTHRESH] <- "";

#Thresholds for Gene names
plt_istumor$Label <- plt_istumor$Gene; #initialize
plt_istumor$Label[(plt_istumor$adj.P.Val >= 0.02)] <- NA; #higher threshold for gene labeling
