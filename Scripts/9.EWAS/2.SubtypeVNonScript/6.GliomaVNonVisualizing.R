...
GliomaVNon <- read.csv("./Script/15.EWAS/SubtypeToNonDMR/BetasGliomaVSNon_results030120.csv", stringsAsFactors = F, row.names =1)
...
plt_istumor2$dir[plt_istumor2$adj.P.Val > 0.01] <- "";
...
final <-volcano_plot + 
  ...
  ggtitle("Glioma vs. Non-Tumor 5hmC ");
...
ggsave(filename = "GliomaVNon.png", final,
...
plt_istumor$dir <- ifelse(plt_istumor$logFC > 0, "Hyper-hydroxymethylated", "Hypo-hydroxymethylated");
plt_istumor$dir[plt_istumor$adj.P.Val > FDR_Thresh] <- "";

#Thresholds for Gene names
plt_istumor$Label <- plt_istumor$Gene; #initialize
plt_istumor$Label[(plt_istumor$adj.P.Val >= 0.1)] <- NA; #higher threshold for gene labeling
...
#  scale_y_continuous(limits=c(0,7.5),breaks=seq(0,7,by=.5) ) +
  theme(plot.title = element_text(hjust = 0.5))+
  ggtitle("Embryonal vs. Nontumor 5hmC ");
